#!/bin/sh
# Fetch the raw mass-setup installer from the latest release and run it.
#
...
#
# MASS_VERSION pins a release tag (e.g. MASS_VERSION=v0.4.0 ... | sh);
# unset, the latest release installs. The variable goes before `sh`, not `curl`.
set -eu

REPO="chinese-room-solutions/mass"
ASSET_PREFIX="mass-setup"
if [ -n "${MASS_VERSION:-}" ]; then
    BASE_URL="https://github.com/$REPO/releases/download/$MASS_VERSION"
else
