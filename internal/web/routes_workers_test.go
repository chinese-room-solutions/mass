...
func TestBenchmarkTarget_MatchWorker(t *testing.T) {
	tests := []struct {
		name      string
		...
		{
			name:      "empty WorkerIDs matches all",
			workerIDs: nil,
			id:        "any-agent",
			want:      true,
		},
		...
		},
		{
			...
			workerIDs: map[string]bool{"agent-1": true, "agent-2": true},
			id:        "agent-1",
			want:      true,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			bt := benchmarkTarget{WorkerIDs: tt.workerIDs}
			require.Equal(t, tt.want, bt.matchWorker(tt.id))
		})
	}
}
...
func (p panicDevices) Devices() []stats.Device { panic("boom") }

type normalDevices struct{ devs []stats.Device }

func (n normalDevices) Devices() []stats.Device { return n.devs }

func TestSafeDevices(t *testing.T) {
	tests := []struct {
		name string
		ag   interface{ Devices() []stats.Device }
		want []stats.Device
	}{
		{
			name: "normal return",
			ag: normalDevices{devs: []stats.Device{
