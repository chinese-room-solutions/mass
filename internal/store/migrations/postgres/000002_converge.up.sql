-- Postgres counterpart of migrations/sqlite/000002_converge.up.sql.
--
...
--
-- Postgres has no CREATE TRIGGER IF NOT EXISTS, so the goqite trigger is
...
CREATE OR REPLACE FUNCTION goqite_update_timestamp() RETURNS TRIGGER AS $$
BEGIN
    NEW.updated := now();
    RETURN NEW;
END;
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE tgname = 'goqite_updated_timestamp'
          AND tgrelid = 'goqite'::regclass
    ) THEN
