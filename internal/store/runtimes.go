...
func (s *Store) SetRuntimeAutoStart(runtimeName string, autoStart bool) error {
	_, err := s.db.Exec(`UPDATE runtimes SET auto_start = ? WHERE runtime_name = ?`, autoStart, runtimeName)
	if err != nil {
...
func (s *Store) GetRuntime(runtimeName string) (RuntimeRow, error) {
	...
	row.InstalledAt, _ = time.Parse(time.RFC3339Nano, ts)
	return row, nil
}
...
func (s *Store) ListRuntimes() ([]RuntimeRow, error) {
	...
	if err != nil {
		return nil, ctxerr.With(fmt.Errorf("listing runtimes: %w", err), nil)
	}
	defer func() {
	...
	for rows.Next() {
		...
		r.InstalledAt, _ = time.Parse(time.RFC3339Nano, ts)
		out = append(out, r)
	}
	return out, rows.Err()
}
...
func (s *Store) DeleteRuntime(runtimeName string) error {
	_, err := s.db.Exec(`DELETE FROM runtimes WHERE runtime_name = ?`, runtimeName)
	if err != nil {
